...
async def get_order(id:int, response:Response, order_repository = Depends(Provide[Container.order_repository_provider])):
    ...
    product = requests.get(f"http://inventory-service:8000/products/{id}").json()
    return {
...
async def set_order(orderdata:OrderModel, response:Response, order_repository = Depends(Provide[Container.order_repository_provider]), event_sender = Depends(Provide[Container.event_sender_provider])):
    ...
    product = requests.get(f"http://inventory-service:8000/products/{orderdata.productId}")
    ...
    requests.post(f"http://inventory-service:8000/products/reserve",json={"productId":orderdata.productId})
    order_id = order_repository.save_order(orderdata)
    event_sender.send_event(orderdata, product_data,order_id)
    return order_id